...
const RegisterPage = () => {
  ...
  const handleRegister = async () => {
    ...
    api
      ...
      .then((res) => {
        setSuccessMessage(res.data.message || "Account created");
        setTimeout(() => {
          navigate("https://webpeek.com/login");
        }, 3000);
      })
      .catch((err) => {
  ...
  return (
    <div className="w-full h-[80vh] flex items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        {successMessage && (
          <div onClick={() => setSuccessMessage(null)} className="fixed top-5  left-1/2 -translate-x-1/2 w-[350px] z-50 px-4 py-3 rounded-lg shadow-lg text-white bg-green-600">
            {successMessage}
          </div>
        )}
        {error && (
          <div
            ...
            className="fixed top-5 left-1/2 -translate-x-1/2 w-[350px] z-50 px-4 py-3 rounded-lg shadow-lg text-white bg-red-600"
          >
            {error}
          </div>
        )}
